...
During the early-access window, \zns{} assumes that access codes are issued only to authorized claimants and remain unavailable to unauthorized parties, and that the Mint's read of the protected-name policy is accurate enough to classify names as open or protected. Disclosure of a valid code, or a false open classification, can allow a protected name to be claimed before general availability. After the fixed general-availability day, these assumptions no longer apply to claim acceptance.
