...
\textbf{Request.} The controller submits the Request specified in Section~\ref{sec:requests} as an Ironwood output to the Mint's treasury account. That note MUST carry the request fee of one US dollar, converted at the rate of the active pricing policy and rounded up to the next 100{,}000-zatoshi step. The fee provides Sybil resistance: an underpaid Request is rejected and its value is retained by the Mint. The fee is not the name price. The Relay is funded from the Mint's treasury account.
...
\textbf{Respond.} The controller returns the memo it received, unchanged, to the same treasury account:

\begin{center}
\texttt{ZNS:otp:<otp>:<name>:<verb>:<ua>}
\end{center}

The Relay and the Respond differ only in sender.
