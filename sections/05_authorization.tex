...
\textbf{Request.} The controller submits the Request specified in Section~\ref{sec:requests} as an Ironwood output to the Mint's treasury account. That note MUST carry the request fee: one US dollar converted at the rate of the active pricing policy and rounded up to the next 100{,}000-zatoshi step. An underpaid Request is rejected and its value is retained by the Mint. The Request funds only the exchange; it is not the name price. The Relay is funded from the Mint's treasury account.
...
\textbf{Respond.} The controller sends the OTP back to the same treasury account. The Respond is byte-for-byte the Relay that delivered the code:

\begin{center}
\texttt{ZNS:otp:<otp>:<name>:<verb>:<ua>}
\end{center}

Relay and Respond are distinguished by direction, not by content.
...
The memo is exactly 512 bytes and zero-padded. The \texttt{otp} message type distinguishes it from a Request. A Request parser MUST reject it; the Mint's parser accepts it as a Respond.
