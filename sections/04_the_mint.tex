...
The Mint is the protocol's single registrar. A Name Note establishes that a transition is bound to an Ironwood output, but whether the transition is allowed depends on the current registry state and active naming policy. The Mint reads ordered Request and Respond memos on Zcash, encoded as specified in Section~\ref{sec:requests}, derives that state, applies the policy, and for each accepted Request or Respond spends a Mint-controlled note to create the successor Name Note.
...
Some names may be designated as protected for a limited early-access window to reduce impersonation, misleading claims of affiliation, and opportunistic registration of names associated with existing identities or projects.

During that window the Mint consults the attested protected-name policy to classify each claim name as open or protected. An open name follows the ordinary claim path. A protected name is accepted only if the claim Request carries the matching access code in the leading slot specified in Section~\ref{sec:requests}. A missing or incorrect code is rejected; the payment is retained by the Mint as for any other rejected claim. If the policy data source is temporarily unavailable, the Mint MUST defer evaluation of that claim until the next tip rather than accept or reject it.

Redemption is derived from registry state: once a protected name has an accepted live Name Note, further claims for that name fail under the ordinary namespace rules. The Mint does not mark codes spent in an external store.

The current \zns{} deployment MUST fix a general-availability day on the Mint's day clock (canonical-chain MTP days since the Mint birthday). At and after that day, access codes are ignored and every name is eligible for the ordinary public claim path. Unredeemed codes become worthless.

Access-code verification is Mint policy under attestation and is not independently reconstructed by the Resolver. Once a protected name is validly claimed, it is governed by the same transition and authorization rules as any other name.
...
\begin{itemize}
  ...
  \item \textbf{Claim eligibility.} A \action{claim} must satisfy the protected-name rules in Claim eligibility above, including any required access code.
  \item \textbf{Payment.} The memo must satisfy the price and payment requirements defined by the active pricing policy. For a \action{claim}, that payment is the Request. For an \action{update} or controller-requested \action{release}, that payment is the Respond specified in Section~\ref{sec:authorization}.
  \item \textbf{Request validity.} The Request or Respond must satisfy Section~\ref{sec:requests}. The name, Unified Address when applicable, requested registration term or extension when applicable, and resulting encoded transition must satisfy the protocol's validity rules.
...
A \action{claim} request specifies the name, the Unified Address, and either a requested registration term or no fixed expiration. The user does not supply an absolute \field{expires\_at} value. A requested registration term MUST NOT exceed 99 years.
...
A registration without fixed expiration (\field{expires\_at} of \texttt{none}) MUST NOT be extended: an update requesting a term against such a registration is refused.

The extension is added to the existing \field{expires\_at}, rather than to the current time. The resulting \field{expires\_at} MUST NOT be more than 99 years ahead of the canonical-chain MTP at which the Mint authorizes the renewal.

An \action{update} with the term \texttt{forever} requests an upgrade of a fixed-term registration to a registration without fixed expiration; an upgrade is not an ordinary \action{update}. The resulting Name Note encodes \field{expires\_at} as \texttt{none}; the registration's liveness obligations are unchanged. The upgrade is subject to the price and payment requirements of the active pricing policy, in addition to the request fee; its payment is the Respond, as for any \action{update}. An upgrade MUST NOT be requested against a registration without fixed expiration.
...
A Request or Relay does not satisfy liveness. A failed, expired, or incomplete Respond does not satisfy liveness. A liveness interval resets only when the resulting \action{update} Name Note is accepted on the canonical Zcash chain.
